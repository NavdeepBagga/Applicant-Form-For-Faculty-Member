...
{	
	...
	function getQuestionInfo($lastQid, $table)
    {
        ...
        {
            $selectQuesInfo = mysql_query("SELECT `qid`, `gid`, `type`, `other` from `$table` "
                            . "WHERE `qid` = '$singleQid'");
    ...
    {			
        ...
        {	       
            if (($singleInfo ['type'] == ";")
                ...
                $selectUnderscoreColm = mysql_query("SELECT `title`, 'scale_id' from `$table`"
                                      . "WHERE `parent_qid` = '$singleInfo[qid]' and `scale_id` = '0' ");
                while($fetchUnderscoreColm = mysql_fetch_array($selectUnderscoreColm))	
                {
                        $selectRepeatColm = mysql_query("SELECT `title` from `$table` "
                                          . "WHERE `parent_qid` = '$singleInfo[qid]' and `scale_id` = '1' ");
                        while($fetchRepeatColm = mysql_fetch_array($selectRepeatColm))
                        {
                            $compoundValue [] = $constSid."X".$singleInfo ['gid'] ."X".$singleInfo ['qid']
                                              . $fetchUnderscoreColm ['title'] ."_".$fetchRepeatColm ['title'];
                        }
                }
              }
            ...
            while($fetchRow = mysql_fetch_array($selectRow))
            {
                if (($fetchRow [$singleColumn] == "A1")
                    ...
                    $selectARows = mysql_query("SELECT answer from `$ansTable` "
                                 . "WHERE `qid` = '$aDivColm[2]' AND `code` = '$fetchRow[$singleColumn]'");		
                ...
                elseif ($fetchRow [$singleColumn] == "Y")
                {
                    ...
                    $selectY=mysql_query("SELECT `type` from $quesTable where `gid`='$yDivColm[1]' AND `qid`='$yDivColm[2]'	");
                    ...
                    $selectYRows = mysql_query("SELECT `question` "
                                 . "from `$quesTable` "
